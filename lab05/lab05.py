def flatten(s):
    ...
    for element in s:
        if type(element) == list:
            s_return.extend(flatten(element))
        else:
            s_return.append(element)
...
def my_reduce(combiner, seq):
    ...
    if len(seq) == 1:
        ...
        ans_return = combiner(seq[0], seq[1])
        for i in range(2, len(seq)):
            ans_return = combiner(ans_return, seq[i])
    return ans_return
...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    city_aimed = make_city('aimed', lat, lon)
    distance_2a = distance(city_aimed, city_a)
    distance_2b = distance(city_aimed, city_b)

    if distance_2a < distance_2b:
...
def count_palindromes(L):
    ...
    2
    """
    #listt = [i for i in L if i[::-1].lower()==i.lower()]
    return len([i for i in L if i[::-1].lower()==i.lower()])
