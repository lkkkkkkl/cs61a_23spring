...
class VendingMachine:
    ...
    def restock(self, num):
        self.storage += num
        return 'Current {} stock: {}'.format(self.product, self.storage)
    
    def add_funds(self, num):
        if self.storage == 0:
            return 'Nothing left to vend. Please restock. Here is your ${}.'.format(num)
        self.account += num
    ...
    def vend(self):
        ...
        if self.account == self.price:
            ...
            diff =  self.account - self.price
            self.storage -= 1
            self.account = 0
            return 'Here is your {} and ${} change.'.format(self.product, diff)
        else:
            diff = self.price - self.account
            return 'Please add ${} more funds.'.format(diff)
