...
def tail_points(opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    num_ones, num_tens = opponent_score % 10, (opponent_score//10) % 10
    if num_ones > num_tens:
        return 2 * (num_ones - num_tens) + 1
    else:
        return 2 * (num_tens - num_ones) + 1        
...
def play(strategy0, strategy1, update,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            score0 = update(dice_count0, score0, score1, dice)
            who = (who+1)%2
        else:
            dice_count1 = strategy1(score1, score0)
            score1 = update(dice_count1, score1, score0, dice)    
            who = (who+1)%2
        # END PROBLEM 5
...
def make_averaged(original_function, total_samples=1000):
    ...
    def averaged(*args):
        ...
        for i in range(total_samples):
            num_all += original_function(*args)
        return num_all/total_samples
    return averaged
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    for num_dice in range(1, 11):
        score_i =  0
        for i in range(total_samples):
                score_i += roll_dice(num_dice, dice)
        list_score.append(score_i)
    return list_score.index(max(list_score)) + 1
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    score_pig = tail_points(opponent_score)
    score_diff= 0
    if perfect_square(score + score_pig):
        score_diff = next_perfect_square(score + score_pig) - score
